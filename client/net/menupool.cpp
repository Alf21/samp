...
void CMenuPool::Process()
{
	...
	if (pControls->wKeys1[16] && !pControls->wKeys2[16]) // Selected an item
	{
		...
		if (row != 0xFF)
		{
			...
			bsSend.Write(row);
			pRak->RPC(&RPC_MenuSelect, &bsSend, HIGH_PRIORITY, RELIABLE, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);
		}
	}
	else if (pControls->wKeys1[15] && !pControls->wKeys2[15]) // Exited
	{
		m_byteExited = 1;
		pRak->RPC(&RPC_MenuQuit, NULL, HIGH_PRIORITY, RELIABLE, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);
	}
